...
each pane's directory in --panes mode).`,
	...
	cmd.Flags().StringVar(&worktreeSetupCmd, "worktree-setup-cmd", "", "shell command run once, synchronously, in a freshly created worktree, right after `git worktree add` succeeds and before the worker's agent is spawned (e.g. copying in gitignored per-developer local config); a non-zero exit fails worktree creation the same way a `git worktree add` failure already does. Empty (default) runs nothing. Without this flag, this repo's .argus/config.yml worktree_setup_cmd wins, then this default. See schemas/config.schema.json's worktree_setup_command for the full cwd/worktree_dir interaction it must respect")
