...
func TestIssuesToTasks(t *testing.T) {
	...
	if len(tasks) != 2 || len(branches) != 2 || len(labels) != 2 {
		t.Fatalf("want 2 tasks/branches/labels, got %d/%d/%d", len(tasks), len(branches), len(labels))
	}
	...
	if labels[0] != "#142" || labels[1] != "#145" {
		t.Errorf("labels: got %v, want bare ticket keys [#142 #145]", labels)
	}
}
...
func TestJiraIssuesToTasks(t *testing.T) {
	...
	tasks, branches, labels, err := jiraIssuesToTasks(context.Background(), io.Discard, f, t.TempDir(), "myrepo", []string{"PROJ-142", "PROJ-145"}, jiraSpawnOpts{}, briefNoteOverride{})
	if err != nil {
	...
	if labels[0] != "PROJ-142" || labels[1] != "PROJ-145" {
		t.Errorf("labels: got %v, want the bare Jira keys [PROJ-142 PROJ-145]", labels)
	}
...
func TestJiraIssuesToTasksAssignsAndTransitionsOnSpawn(t *testing.T) {
	...
	_, _, _, err := jiraIssuesToTasks(context.Background(), io.Discard, f, t.TempDir(), "myrepo", []string{"PROJ-142", "PROJ-145"},
...
func TestJiraIssuesToTasksAbortsOnAssignFailure(t *testing.T) {
	...
	_, _, _, err := jiraIssuesToTasks(context.Background(), io.Discard, f, t.TempDir(), "myrepo", []string{"PROJ-1"}, jiraSpawnOpts{assignToCaller: true}, briefNoteOverride{})
	if err == nil {
...
func TestJiraIssuesToTasksAbortsOnTransitionFailure(t *testing.T) {
	...
	_, _, _, err := jiraIssuesToTasks(context.Background(), io.Discard, f, t.TempDir(), "myrepo", []string{"PROJ-1"}, jiraSpawnOpts{transition: "Bogus"}, briefNoteOverride{})
	if err == nil {
