...
func TestEnsureExactAllowCreatesFileAndDir(t *testing.T) {
	path := filepath.Join(t.TempDir(), ".claude", "settings.json")
	added, err := EnsureExactAllow(path, "Bash(git push --force-with-lease origin feat-x)")
	if err != nil {
		t.Fatalf("EnsureExactAllow: %v", err)
	}
	if !added {
		t.Fatal("expected added=true for a fresh file")
	}
	covered, matches, err := Check(path)
	if err != nil {
		t.Fatalf("Check after EnsureExactAllow: %v", err)
	}
	if covered {
		t.Errorf("EnsureExactAllow's entry should not satisfy argus's own Covers check, got matches %v", matches)
	}
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(data), "Bash(git push --force-with-lease origin feat-x)") {
		t.Errorf("settings file missing the exact entry: %s", data)
	}
}

func TestEnsureExactAllowIsIdempotent(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.json")
	entry := "Bash(git push --force-with-lease origin feat-x)"
	writeSettings(t, path, `{"permissions":{"allow":["`+entry+`"]}}`)
	before, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	added, err := EnsureExactAllow(path, entry)
	if err != nil {
		t.Fatalf("EnsureExactAllow: %v", err)
	}
	if added {
		t.Error("expected added=false when the exact entry is already present")
	}
	after, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	...
	if string(before) != string(after) {
		t.Error("EnsureExactAllow rewrote the file even though nothing needed to change")
	}
}

func TestEnsureExactAllowDoesNotTreatBroaderEntryAsCovering(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.json")
	writeSettings(t, path, `{"permissions":{"allow":["Bash(git push:*)"]}}`)
	added, err := EnsureExactAllow(path, "Bash(git push --force-with-lease origin feat-x)")
	if err != nil {
		t.Fatalf("EnsureExactAllow: %v", err)
	}
	if !added {
		t.Error("expected added=true: EnsureExactAllow is a literal membership check, not Covers-style specificity")
	}
}

func TestEnsureExactAllowPreservesUnrelatedKeys(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.json")
	writeSettings(t, path, `{
  "model": "opus",
  "permissions": {
	...
	if _, err := EnsureExactAllow(path, "Bash(git push --force-with-lease origin feat-x)"); err != nil {
		t.Fatalf("EnsureExactAllow: %v", err)
	}
	data, err := os.ReadFile(path)
	if err != nil {
...
func TestRemoveAskDeletesMatchingEntry(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.local.json")
	writeSettings(t, path, `{"permissions":{"ask":["Bash(git commit:*)","Bash(git push:*)"]}}`)
	removed, err := RemoveAsk(path, "Bash(git push:*)")
	if err != nil {
	...
	ask, ok := perms["ask"].([]any)
	if !ok || len(ask) != 1 || ask[0] != "Bash(git commit:*)" {
		t.Errorf("ask = %v, want only the untouched commit entry", ask)
	}
}

func TestRemoveAskIsIdempotentWhenAbsent(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.local.json")
	writeSettings(t, path, `{"permissions":{"ask":["Bash(git commit:*)"]}}`)
	before, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	...
	if removed {
		t.Error("expected removed=false: the entry was never present")
	}
	after, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	...
	if string(before) != string(after) {
		t.Error("RemoveAsk rewrote the file even though nothing needed to change")
	}
}

func TestRemoveAskMissingFileIsNotAnError(t *testing.T) {
	...
	if removed {
		t.Error("expected removed=false for a missing file")
	}
	if _, err := os.Stat(path); err == nil {
		t.Error("RemoveAsk should not create a file when there's nothing to remove")
	}
}

func TestRemoveAskPreservesUnrelatedKeys(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.local.json")
	writeSettings(t, path, `{
  "permissions": {
    "allow": ["Bash(git status*)"],
	...
	if _, err := RemoveAsk(path, "Bash(git push:*)"); err != nil {
		t.Fatalf("RemoveAsk: %v", err)
	}
	data, err := os.ReadFile(path)
	if err != nil {
