...
// at this point, only the gate's.
func BuildPlan(workers []Worker, base string, project protocol.PhaseConfig, baseAllow, extraAllow []string, gateVerifyCommand string) ([]WorkerPlan, error) {
	...
	for i := range workers {
		...
		plans[i] = WorkerPlan{
			Worker:   w,
			Settings: settingsFor(w.Worktree, project, baseAllow, extraAllow, rebaseAllow),
...
func Run(ctx context.Context, cfg *Config, workers []Worker, dryRun bool) error {
	plans, err := BuildPlan(workers, cfg.Base, cfg.RepoPhases, cfg.RepoAllow, cfg.ExtraAllow, cfg.GateVerifyCommand)
	if err != nil {
		return err
	}
...
func provisionWorktree(ctx context.Context, cfg *Config, p *WorkerPlan) error {
	...
	if err := WriteSettings(p.Worktree, cfg.RepoPhases, cfg.RepoAllow, cfg.ExtraAllow, rebaseAllow); err != nil {
